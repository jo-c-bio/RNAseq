...
dataPrep<-function(comp1, comp2){
  #checking differences-the cWTvsvWT has different amount of genes to the other comparisions.
  ...
  matrix1<-bind_cols(comp1,comp2)
  rownames(matrix1) <- matrix1$comp1_ensembl_gene_id
  matrix <- within(matrix1, rm("comp1_ensembl_gene_id","comp2_ensembl_gene_id"))
  
  #This does notget rid of gene repeats, but rather genes that have the exact same expression set as another. 
...
#comp1 <- read.csv('/Users/joannachustecki/Documents/RNAseqAnalysis/VRN2_RNA_seq_data_Untreated_vs_Treated/CWTvsVWT_0FC.csv')
#comp2 <- read.csv("/Users/joannachustecki/Documents/RNAseqAnalysis/VRN2_RNA_seq_data_Untreated_vs_Treated/Cprt6vsVprt6_0FC.csv")

currentWD <- getwd()

###extra for hypoxia data
#comp1 <- read.csv("/Users/joannachustecki/Documents/RNAseqAnalysis/VRN2_RNA_seq_data_Untreated_vs_Treated/Cprt6vsVprt6_0FC.csv")
#comp2 <- read.csv("/Users/joannachustecki/Documents/RNAseqAnalysis/withHypoxiaData/Homeostatic_response_paper_ESM/TranscriptomeData-Table1.csv",header=TRUE)

#currentWD <- getwd()

#we'll split the hypoxia data frame into the comparisons we want, that is WTHS_vs_WTNS and prt6HS_vs_prt6NS
#comp2.b.prt6HS_vs_prt6NS <- data.frame(comp2$locus,comp2$prt6HSvsprt6NS_Log2signal)

#colnames(comp2.b.prt6HS_vs_prt6NS) <- c("geneName","log2FoldChange")

#comp2<-comp2.b.prt6HS_vs_prt6NS

#comparison1<-"cPRT6_vs_vPRT6"
#comparison2<-"prt6HS_vs_prt6NS"
...
#if you want to run up or down only genes, edit the filters here

y2 <- list(unlist(y$comp1x[,1]), unlist(y$comp2x[,1]))
           #,
           #cPRT6_vs_vPRT6 = unlist(y$comp3x[,1]),cDM_vs_vDM = unlist(y$comp4x[,1]))
names(y2) <- c(comparison1, comparison2)
...
venn
ggsave(paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison1,"_",comparison2,"_FC",f,".pdf",sep=""),venn)

#"#868686FF", "#CD534CFF"
#can't yet find a way to get the same/diff lists out if the ggvenn function, so we'll use 
#All DEGs from comp1, regardless of comp2.(i.e. all WT genes as a control.)
comp1only <- y2[[1]]
#All DEGs from comp2, regardless of comp1.(i.e. all prt6 genes changed, just incase I need it.)
comp2only <- y2[[2]]
#unique to comparison 1
comp1unique <- setdiff(y2[[1]],y2[[2]])
#unique to comparison 2
comp2unique <- setdiff(y2[[2]],y2[[1]])
#intersection
intersect <- intersect(y2[[1]],y2[[2]])
#Take these forward for GO term analysis
write.csv(comp1only,paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison1,"Only/geneList-",comparison1,"Only_",f,"FC.csv",sep=""))
write.csv(comp2only,paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison2,"Only/geneList-",comparison2,"Only_",f,"FC.csv",sep=""))
write.csv(comp1unique,paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison1,"unique_vs_",comparison2,"/geneList-",comparison1,"unique_vs_",comparison2,"_",f,"FC.csv",sep=""))
write.csv(comp2unique,paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison2,"unique_vs_",comparison1,"/geneList-",comparison2,"unique_vs_",comparison1,"_",f,"FC.csv",sep=""))
write.csv(intersect,paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison2,"intersect",comparison1,"/geneList-",comparison2,"intersect",comparison1,"_",f,"FC.csv",sep=""))

#Also print these aswe want to make sure the significance and fold change values are correct going forward
write.csv(comp1[match(comp1only,comp1$ensembl_gene_id),] , paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison1,"Only/fullList-",comparison1,"Only_",f,"FC.csv",sep=""))
write.csv(comp2[match(comp2only,comp2$ensembl_gene_id),] , paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison2,"Only/fullList-",comparison2,"Only_",f,"FC.csv",sep=""))
write.csv(comp1[match(comp1unique,comp1$ensembl_gene_id),] , paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison1,"unique_vs_",comparison2,"/fullList-",comparison1,"unique_vs_",comparison2,"_",f,"FC.csv",sep=""))
write.csv(comp2[match(comp2unique,comp2$ensembl_gene_id),] , paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison2,"unique_vs_",comparison1,"/fullList-",comparison2,"unique_vs_",comparison1,"_",f,"FC.csv",sep=""))
write.csv(comp1[match(intersect,comp1$ensembl_gene_id),] , paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",comparison2,"intersect",comparison1,"/fullList-",comparison2,"intersect",comparison1,"_",f,"FC.csv",sep=""))
#^the intersect full list is taken from cWTvsvWT,  not prt6 list so need to be careful when using. 

#heatmap

matrix<-dataPrep(comp1,comp2)
...
#We have chosen scale='none' for the heatmap as we don;t want the heatmap function to scale our fold change values relative to each other, we want the raw FC.
  pHM<-pheatmap(matrix, show_rownames = T, 
                #color = cols,
                fontsize_row = 0.5,
                scale = "none", border_color = NA,
                fontsize_col = 10,treeheight_col = 0,
                labels_col= c(comparison1,comparison2),height=30,width=10, main = title)
  ...
  print(vennpHM)
  ggsave(paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/fc",f,outFileName,".pdf",sep=""),vennpHM, height=10,width=5)
  write.csv(rownames(matrix),paste(currentWD,"/GOTermAnalysis/",comparison1,"_",comparison2,"/",f,"FC",outFileName,".csv",sep=""))
...
##generateHM(matrixFull, paste("All genes of both comparisons pass significance threshold,","\n","no fold change filter"), "NoVenn")
generateHM(matrixIntersect, "All genes filtered for significance and fold change", "YesVenn",paste(comparison1,comparison2,"intersect",sep=""))
